...
bool GCodeQueue::RingBuffer::enqueue(const char *cmd, bool skip_ok/*=true*/
  ...
  // SERIAL_ECHOLNPAIR(" index_r=: ", index_r);
  // SERIAL_ECHOLNPAIR(" index_w=: ", index_w);
...
void GCodeQueue::enqueue_now_P(PGM_P const pgcode) {
  size_t i = 0;
  PGM_P p = pgcode;
  if (ring_buffer.full()) return; //rock——20220815
  for (;;) {
    char c;
    while ((c = pgm_read_byte(&p[i])) && c != '\n') i++;
    char cmd[i + 1];
...
void GCodeQueue::get_serial_commands() {
  ...
  inline void GCodeQueue::get_sdcard_commands()
  {
    ...
    if (!IS_SD_FETCHING()|| !IS_SD_INSERTED())
    {
      return;
    }
    int sd_count = 0;
...
void GCodeQueue::advance() {
  ...
  if(ring_buffer.length != 0) ring_buffer.advance_pos(ring_buffer.index_r, -1);
}

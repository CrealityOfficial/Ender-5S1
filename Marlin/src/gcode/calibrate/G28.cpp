...
void GcodeSuite::G28()
{
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    // Home Z last if homing towards the bed
      ...
      if (doZ) {
        ...
        SERIAL_ECHO("\r\n-------homeaxis2"); 
        probe.move_z_after_homing();   //rock_20220824
      }
    #endif
  ...
  if (st_bedNozzleHeightCal.isBedLeveling == false)
#endif
  ...
  // SERIAL_ECHOLNPAIR(" leveing_flag=: ", leveing_flag);
  // if(leveing_flag) //如果在调平中就不恢复自动补偿功能
  //  process_subcommands_now_P(PSTR("M420 S0"));
  //  else 
  //  process_subcommands_now_P(PSTR("M420 S1"));//rock

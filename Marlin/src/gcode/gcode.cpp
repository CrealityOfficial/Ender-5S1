...
static int8_t s_cloudPLRFirstG28 = 0;
// Inactivity shutdown
  ...
  */
  ...
  switch (parser.command_letter) {
    ...
    case 'G': switch (parser.codenum) {
      ...
      case 28:
        G28();
        if ((false == PoweroffContinue) && (2 == g_cloudPLRStatusValue) && (0 == s_cloudPLRFirstG28)) {
          s_cloudPLRFirstG28 = 1;
        }
        #if HAS_LEVELING
 ...
 */
extern bool rts_test;
void GcodeSuite::process_next_command() {
  // if(rts_test) {SERIAL_ECHO_MSG("process_next_command() 1"); delay(10);}
  GCodeQueue::CommandLine &command = queue.ring_buffer.peek_next_command();

  // if(rts_test) SERIAL_ECHO_MSG("process_next_command() 2, command.buffer:", command.buffer, ",queue.ring_buffer.index_r:", queue.ring_buffer.index_r);
  ...
  // if(rts_test) SERIAL_ECHO_MSG("process_next_command() 3");

  // Parse the next command in the queue
  parser.parse(command.buffer);
  // if(rts_test) SERIAL_ECHO_MSG("process_next_command() 4");
  process_parsed_command();

  // if(rts_test) SERIAL_ECHO_MSG("process_next_command() 5");
}

...
uint8_t g_soundSetOffOn; // 1：开声音，2：关声音
...
void startOrResumeJob() {
  ...
  inline void abortSDPrinting() {
    ...
    TERN_(POWER_LOSS_RECOVERY, recovery.purge());
      ...
      SERIAL_ECHO_MSG("abortSDPrinting");
      // queue.inject_P(PSTR(EVENT_GCODE_SD_ABORT));
      // if((axis_is_trusted(X_AXIS)) && (axis_is_trusted(Y_AXIS)) && (axis_is_trusted(Z_AXIS)))
      {
        SERIAL_ECHO_MSG("abortSDPrinting EVENT_GCODE_SD_ABORT");
...
inline void manage_inactivity(const bool ignore_stepper_queue=false) {
  ...
  if (gcode.stepper_inactive_time) {
    ...
    else if (!parked_or_ignoring && gcode.stepper_inactive_timeout()&&(!card.isPrinting())&&(!card.isPaused())) 
    {
      //isPrinting()   isPaused()  rock_20220815
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  // Handle SD Card insert / remove
   if(!home_flag&&!G29_flag)//为了解决调平和回零中，插拔卡会影响探针，报错203的问题
  ...
  TERN(RTS_AVAILABLE, RTSUpdate(), ui.update());
    ...
    if(0 == READ(PROBE_ACTIVATION_SWITCH_PIN))
    {
      if(home_flag)
      {
...
void setup() {
  ...
  st_bedNozzleHeightCal.bedNozzleHeightCalFinishFlag = true;//强制认为测量高度完成
  st_bedNozzleHeightCal.isBedLeveling=false;     
  gcode.process_subcommands_now_P(PSTR("M420 S1")); // 上电后打开自动调平
}


/*
函数名：SysTimeMonitor
函数功能：检视函数，例如断料检测的喷嘴温度清零
传入参数：无
返回值：无
作者：110235龙志行
备注：这个函数要放在idle函数里面，因为有些状态是在idle里面死循环的
*/
void SysTimeMonitor()
{
  
  // RunoutOvertimeMonitor();
}
/**
...
void loop()
{
  // int *test = (int *)0xcccccccc;
  // *test = 1;
  // SERIAL_ECHO_MSG("test", *test);
  do
  {
    idle();
    #if ENABLED(SDSUPPORT)

...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    #if ENABLED(BLTOUCH_AND_Z_LIMIT)
  EEPROM_WRITE(rtscheck.zCoordinateOffset);         // Z轴空间坐标差
 #endif
    //
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      EEPROM_READ(rtscheck.zCoordinateOffset); //rock_20220730
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    SERIAL_ECHOLNPAIR("  sound Set OffOn", int(g_soundSetOffOn));
    ...
    SERIAL_ECHOLNPAIR("  rtscheck.zCoordinateOffset ! ", int(rtscheck.zCoordinateOffset));
  }

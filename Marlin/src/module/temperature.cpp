...
// public:
  ...
  void Temperature::PID_autotune(const celsius_t target, const heater_id_t heater_id, const int8_t ncycles, const bool set_result/*=false*/) {
    ...
    while (wait_for_heatup) {
        ...
        if (!heating && current_temp < target && ELAPSED(ms, t1 + 5000UL)) {
          ...
          if (cycles > 0) {
            ...
            if (cycles > 2) {
              ...
              SERIAL_ECHOLNPAIR("pf=", pf, ",df=", df);
              SERIAL_ECHOLNPAIR("d=", d, ",maxT=", maxT, ",t_low=", t_low, ",t_high=", t_high);
              if (ischamber || isbed)
      ...
      if (ELAPSED(ms, ui_next_temp_ms)) { // 曲线
        ...
        if (g_uiAutoPIDRuningDiff == 1) { 
          ...
          SERIAL_ECHOLNPAIR("cnt ",g_uiCurveDataCnt);
        ...
        SERIAL_ECHOLNPAIR("g_uiAutoPIDRuningDiff ",g_uiAutoPIDRuningDiff);
      }
          ...
          if (BOTH(WATCH_BED, WATCH_HOTENDS) || isbed == DISABLED(WATCH_HOTENDS) || ischamber == DISABLED(WATCH_HOTENDS)) {
            if (!heated) {                                            // If not yet reached target...
              ...
              else if (ELAPSED(ms, temp_change_ms))                   // Watch timer expired
              {
                  ...
                  if(heater_id == 0)
                  {
                    rtscheck.RTS_SndData(Error_07, ABNORMAL_PAGE_TEXT_VP);
                  }
                  else
                  {
                    rtscheck.RTS_SndData(Error_10, ABNORMAL_PAGE_TEXT_VP);
                  }
                #endif
      ...
      if (cycles > ncycles && cycles > 2) {
        ...
        g_autoPID.Kp = tune_pid.Kp;
        g_autoPID.Ki = tune_pid.Ki;
        g_autoPID.Kd = tune_pid.Kd;
        SERIAL_ECHOLNPAIR("g_autoPID.Kp ", g_autoPID.Kp);
        SERIAL_ECHOLNPAIR("g_autoPID.Ki ", g_autoPID.Ki);
        SERIAL_ECHOLNPAIR("g_autoPID.Kd ", g_autoPID.Kd);
...
void Temperature::max_temp_error(const heater_id_t heater_id)
{
  ...
  #elif ENABLED(RTS_AVAILABLE) && (HAS_HOTEND || HAS_HEATED_BED)
    // rtscheck.RTS_SndData(ExchangePageBase + 31, ExchangepageAddr);
    // change_page_font = 31;
    rtscheck.RTS_SndData(ExchangePageBase + 41, ExchangepageAddr);
    change_page_font = 41;
    if(heater_id == 0)
    {
      rtscheck.RTS_SndData(Error_08, ABNORMAL_PAGE_TEXT_VP);
    }
    else
    {
      rtscheck.RTS_SndData(Error_11, ABNORMAL_PAGE_TEXT_VP);
    }
  #endif
  ...
  millis_t ms = millis();
    ...
    HOTEND_LOOP() {
      ...
      temp_hotend[e].soft_pwm_amount = (temp_hotend[e].celsius > temp_range[e].mintemp || is_preheating(e)) && temp_hotend[e].celsius < temp_range[e].maxtemp ? (int)get_pid_output_hotend(e) >> 1 : 0;
        ...
        if (watch_hotend[e].elapsed(ms))
        {
          // Enabled and time to check?
          if (watch_hotend[e].check(degHotend(e)))
          {
            // Increased enough?
            // If temp reached, turn off elapsed check
  ...
  void Temperature::tr_state_machine_t::run(const_celsius_float_t current, const_celsius_float_t target, const heater_id_t heater_id, const uint16_t period_seconds, const celsius_t hysteresis_degc) {
    ...
    switch (state) {
      ...
      case TRRunaway:
          ...
          if(heater_id == 0)
          {
            rtscheck.RTS_SndData(Error_06, ABNORMAL_PAGE_TEXT_VP);
          }
          else
          {
            rtscheck.RTS_SndData(Error_09, ABNORMAL_PAGE_TEXT_VP);
          }
        #endif
        TERN_(DWIN_CREALITY_LCD, DWIN_Popup_Temperature(0));
        _temp_error(heater_id, str_t_thermal_runaway, GET_TEXT(MSG_THERMAL_RUNAWAY));

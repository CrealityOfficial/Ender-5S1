...
void prepare_line_to_destination() {
  ...
  void do_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s=0.0, const bool final_approach=true) {
    ...
    if (DEBUGGING(LEVELING)) {
      ...
      if((axis == Z_AXIS) && (1 == READ(PROBE_ACTIVATION_SWITCH_PIN)))
      {
  ...
  void set_axis_never_homed(const AxisEnum axis) {
    ...
    SERIAL_ECHO_MSG("set_axis_never_homed set_axis_untrusted");
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    if (bump) {
      ...
      // if(axis == Z_AXIS)  //rock_20220801 手动对高功能。
      // {
...
//         SERIAL_ECHOLNPAIR(" current_position.z_old=: ", current_position.z);
//         current_position.z -= probe.offset.z;
//         SERIAL_ECHOLNPAIR(" probe.offset.z=: ", probe.offset.z);
//         SERIAL_ECHOLNPAIR(" current_position.z_new=: ", current_position.z);
...
// //  #endif
  ...
  #elif HAS_BED_PROBE && Z_HOME_DIR > 0 // 正方向回原点进行Z轴补偿
    if (axis == Z_AXIS) {
      #if ENABLED(RTS_AVAILABLE) && ENABLED(BLTOUCH) && Z_HOME_DIR > 0
      if(st_bedNozzleHeightCal.bedNozzleHeightCalFinishFlag == true)
        current_position.z -= (st_bedNozzleHeightCal.zCoordinateOffset + probe.offset.z);
      // #else
      //   current_position.z -= probe.offset.z;
      #endif
    } 
...
//   if (ENABLED(USER_LEVEL_DEUBG)) {
//     SERIAL_ECHOLNPAIR("\r\nhomeaxis_bl start current_position.z * 100 = ", current_position.z*100);
//   }


//   if (ENABLED(USER_LEVEL_DEUBG)) DEBUG_ECHOLNPAIR(">>> homeaxis(", axis_codes[axis], ")");
...
//   // Fast move towards endstop until triggered
//   if (ENABLED(USER_LEVEL_DEUBG)) DEBUG_ECHOLNPGM("Home 1 Fast:");

//   #if BOTH(HOMING_Z_WITH_PROBE, BLTOUCH)
//     if (axis == Z_AXIS && bltouch.deploy()) return VAULE_INVALIDE_8BIT; // The initial DEPLOY
//   #endif
...
//   rtscheck.SetzCoordinateOffset(target2[axis] + target1[axis]);
...
//     SERIAL_ECHOLNPAIR("\r\nhomeaxis_bl Home 1-1 Fast: zCoordinateOffset * 100 = ", rtscheck.zCoordinateOffset*100);
//     #elif ENABLED(DWIN_CREALITY_SERMOON_V2)
...
//   // 获取z轴两坐标系的差值
//   #if ANY( DWIN_CREALITY_CR200B_PRO)
//   rtscheck.SetzCoordinateOffset(rtscheck.GetzCoordinateOffset() + /*target3[axis] + */target4[axis]);
//   #elif ENABLED(DWIN_CREALITY_SERMOON_V2)
...
//   if (ENABLED(USER_LEVEL_DEUBG)) {
//     #if ANY(DWIN_CREALITY_SERMOON_D3, DWIN_CREALITY_CR200B_PRO)
//     SERIAL_ECHOLNPAIR("\r\nhomeaxis_bl Home 1-2 Fast: zCoordinateOffset * 100 = ", rtscheck.zCoordinateOffset*100,
//                       "\r\nhomeaxis_bl Home 1-2 Fast:  coordinateOffset * 100 = ", coordinateOffset * 100);
//     #elif ENABLED(DWIN_CREALITY_SERMOON_V2)
...
//   if (bump) {
//     // Move away from the endstop by the axis HOMING_BUMP_MM
//     if (ENABLED(USER_LEVEL_DEUBG)) DEBUG_ECHOLNPGM("Move Away:");
...
//     // Slow move towards endstop until triggered
//     if (ENABLED(USER_LEVEL_DEUBG)) DEBUG_ECHOLNPGM("Home 2 Slow:");   //第二次测量
...
//     // 第二次测量值
//     #if ANY(DWIN_CREALITY_CR200B_PRO)
//     rtscheck.SetzCoordinateOffset(rtscheck.GetzCoordinateOffset() - (target5[axis] + target6[axis]));
//     #elif ENABLED(DWIN_CREALITY_SERMOON_V2)
...
//     if (ENABLED(USER_LEVEL_DEUBG)) {
//       #if ANY(DWIN_CREALITY_SERMOON_D3, DWIN_CREALITY_CR200B_PRO)
//       SERIAL_ECHOLNPAIR("\r\nhomeaxis_bl Home 2 Slow: zCoordinateOffset * 100 = ", rtscheck.zCoordinateOffset*100,
...
//   // Put away the Z probe
//   #if HOMING_Z_WITH_PROBE
//     if (axis == Z_AXIS && probe.stow()) return VAULE_INVALIDE_8BIT;
//   #endif

...
#define RUNOUT_TIME_MAX (1000 * 5 * 60)
#include "runout.h"
...
uint32_t g_runOutTimeMs = 0;
uint32_t g_runOutTimeFlag = 0;
void event_filament_runout(const uint8_t extruder)
{
  #if ENABLED(RTS_AVAILABLE)
    if((G29_flag == true) || (home_flag == true))
    {
      return;
    }
  #endif
...
/*
函数名：RunoutOvertimeMonitor
函数功能：断料时间达到g_runOutTimeMs毫秒后，将喷嘴温度降为零，避免耗材碳化
传参：无
返回值：无
作者：110235龙志行
*/
void RunoutOvertimeMonitor()
{
  uint32_t deltaTime;
  if (change_page_font == 7) { // ͣ停在断料界面
    if (g_runOutTimeFlag == 0) { // 第一进入断料获取初始时间
      g_runOutTimeMs = millis();
      g_runOutTimeFlag = 1;
      SERIAL_ECHOPGM("g_runOutTimeMs = millis()");
    }
    deltaTime = millis() - g_runOutTimeMs; // 获取时间
    if (deltaTime > RUNOUT_TIME_MAX) { // 断料到达5 min温度设置为零
      thermalManager.setTargetHotend(0, 0); // 温度设为零，避免耗材碳化
      rtscheck.RTS_SndData(0, HEAD_SET_TEMP_VP);
      // SERIAL_ECHOPGM("thermalManager.setTargetHotend(0, 0)");
    }
  } else { // 把断料标志位清零
      g_runOutTimeFlag = 0;
  }
